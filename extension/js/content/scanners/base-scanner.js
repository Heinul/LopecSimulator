...
LopecScanner.Scanners.BaseScanner = (function() {
  ...
  async function finishScan() {
    ...
    try {
      ...
      let apiKey = null;
      try {
        // chrome.storage.local에서 API 키 조회
        await new Promise(resolve => {
          chrome.storage.local.get(['lostarkApiKey'], function(result) {
            if (result && result.lostarkApiKey) {
              apiKey = result.lostarkApiKey;
              console.log('API 키 가져옴 - 시작 부분:', apiKey.substring(0, 5));
            }
            resolve();
          });
        });
      } catch (e) {
        console.warn('API 키 가져오기 실패:', e);
      }
      
      // API 키가 있으면 각인서 가격 정보 가져오기
      if (apiKey) {
        // 각인서 관련 스캔 결과만 필터링
        const engravingResults = Object.entries(state.scanResults).filter(([key, value]) => 
          key.startsWith('engraving-') && value.type === 'engraving'
        );
        
        if (engravingResults.length > 0) {
          console.log(`각인서 관련 결과 ${engravingResults.length}개 발견, 가격 정보 가져오기 시도...`);
          
          // API 호출을 위한 각인 이름 중복 제거
          const uniqueEngravings = new Set();
          engravingResults.forEach(([_, value]) => {
            uniqueEngravings.add(value.engravingName);
          });
          
          console.log(`고유한 각인 이름 ${uniqueEngravings.size}개 발견:`, Array.from(uniqueEngravings));
          
          try {
            // EngravingCalculator 모듈이 있는지 확인
            if (typeof LopecScanner.Data !== 'undefined' && 
                typeof LopecScanner.Data.EngravingCalculator !== 'undefined') {
              
              const calculator = LopecScanner.Data.EngravingCalculator;
              
              // 고유한 각 각인서별로 가격 정보 가져오기
              for (const engravingName of uniqueEngravings) {
                try {
                  // 한 번의 API 호출로 모든 등급의 각인서 가격 정보 가져오기
                  // 파라미터를 2개(CategoryCode + ItemName)만 사용하기 위해 null 전달
                  const priceResult = await calculator.getAllEngravingPrices(
                    engravingName, // 각인서 이름만 사용
                    null,          // 등급 파라미터 사용 안함
                    apiKey
                  );
                  
                  if (priceResult) {
                    console.log(`각인서 '${engravingName}' 가격 정보 가져오기 성공:`, priceResult);
                    
                    // 해당 각인서 관련 모든 결과에 가격 정보 추가
                    engravingResults.forEach(([key, value]) => {
                      if (value.engravingName === engravingName) {
                        // 변화 정보 계산
                        const fromGrade = value.fromGrade;
                        const fromLevel = value.fromLevel;
                        const toGrade = value.toGrade;
                        const toLevel = value.toLevel;
                        ...
                        calculator.calculatePrice(
                          ...
                          if (priceInfo && priceInfo.success) {
                            // 결과에 가격 정보 추가
                            state.scanResults[key].priceInfo = priceInfo;
                            console.log(`각인 '${engravingName}' ${fromGrade} Lv.${fromLevel} -> ${toGrade} Lv.${toLevel} 가격 계산:`, 
                              priceInfo.price ? `${priceInfo.price} 골드` : '가격 없음');
                          }
                        }).catch(e => {
                          console.error(`각인서 가격 계산 오류 (${engravingName}):`, e);
                        });
                      }
                    });
                  }
                } catch (e) {
                  console.error(`각인서 '${engravingName}' 가격 정보 가져오기 오류:`, e);
                }
              }
            } else {
              console.warn('EngravingCalculator 모듈을 찾을 수 없습니다.');
            }
          } catch (e) {
            console.error('각인서 가격 정보 처리 중 오류:', e);
          }
        }
      } else {
        console.log('API 키가 없어 각인서 가격 정보 가져오기 스킵');
      }
    } catch (e) {
      console.error('각인서 가격 정보 처리 오류:', e);
    } finally {
      // 확장 프로그램에 완료 알림
